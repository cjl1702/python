...
#获取影视评论信息
#date:2018/11/9
def comments(film_id):

    import urllib.request
    from bs4 import BeautifulSoup as bs

    # 评论请求地址
    url = 'https://movie.douban.com/subject/'+ film_id +'/comments?start=0&limit=20'
    requrest = urllib.request.Request(url)
    response = urllib.request.urlopen(requrest)
    data = response.read()
...
def douban_wcloud(content):
    ...
    # 与停用词对比，去掉虚词
    # quoting = 3 全不引用
    stopwords = pd.read_csv("../stopwords.txt", index_col=False, quoting=3, sep="t", names=['stopword'],
                            encoding='gb18030')
...
if __name__ == '__main__':
    result =   comments('25917789')
    douban_wcloud(result)
